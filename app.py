...
if readfrom == 'db':
    ...
    df_main, df_vuln = datafiles.read_data_files()
...
def callback_comptab_selcomp_button(nclicks, cdata, rows):
    ...
    if rows:
        return comptab.create_comptab_card_comp(
            df_comp[df_comp['compVerId'] == cdata[rows[0]]['compVerId']]), 'tab_comp_subdetail'

    return comptab.create_comptab_card_comp(None), 'tab_comp_subsummary'
...
def callback_vulntab_selvuln_button(nclicks, cdata, rows):
    ...
    if rows:
        ...
        return vulntab.create_vulntab_card_vuln(df_vuln[df_vuln['vulnId'] == cdata[rows[0]]['vulnId']])

    return vulntab.create_vulntab_card_vuln(None)
...
def callback_projtab_selproj_button(nclicks, cdata, rows):
    ...
    if rows:
        return projtab.create_projtab_card_proj(
            df_proj[df_proj['projVerId'] == cdata[rows[0]]['projVerId']]), 'tab_proj_subdetail'

    return projtab.create_projtab_card_proj(None), 'tab_proj_subsummary'
